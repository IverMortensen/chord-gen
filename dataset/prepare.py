...
def encode_split(df: pd.DataFrame, stoi: dict, bos_id: int, eos_id: int,
                 unk_id: int) -> tuple[np.ndarray, int]:
    """Encode all songs in a split into one concatenated uint16 array.

    Each song becomes [<bos>] + token_ids + [<eos>]. Returns the array and a
    count of tokens that fell back to <unk> (should be ~0 since the vocab was
    built from this same data; a nonzero count signals a vocab/data mismatch).
    """
    ids: list[int] = []
    unk_hits = 0
...
def main(argv=None):
    ...
    # --- Encode --------------------------------------------------------------
    train_ids, train_unk = encode_split(train_df, stoi, bos_id, eos_id, unk_id)
    val_ids, val_unk = encode_split(val_df, stoi, bos_id, eos_id, unk_id)
    ...
    # meta.pkl: exactly the keys nanoGPT's train.py and sample.py read.
